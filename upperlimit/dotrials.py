...
mres.SetParameters(0.0007202,0.03815)
pdf = TF1("eventpdf","max(0,pol2)",0.02,0.06)
pdf.SetParameters(-0.0484,3.542,-46.99)
...
pvalHist = TH1D("pval","pval",10000,0,1.0)
allpvalHist = TH1D("allpval","allpval",10000,0,1.0)
pvalCdfHist = TH1D("pvalCdf","pvalCdf",10000,0,1.0)
...
for runnum in xrange(0,n):
    ...
    data.FillRandom("eventpdf",rand.Poisson(12))
...
for i in xrange(0,pvalHist.GetXaxis().GetNbins()+1):
    ...
    pvalCdfHist.SetBinContent(i,total/pvalHist.Integral())
...
c.Print(remainder[0]+".pdf")
pvalCdfHist.GetXaxis().SetRangeUser(1e-3,0.5)
pvalCdfHist.GetYaxis().SetRangeUser(1e-3,1)
c.SetLogy()
c.SetLogx()
pvalCdfHist.Draw()
c.Print(remainder[0]+".pdf")
