...
PARS={
    ...
    'FIELDMAP':'209acm2_5kg_corrected_unfolded_scaled_1.04545.dat',
    'TAPETOPDIR':'/mss/hallb/hps/physrun2016',
    'DISKTOPDIR':'/work/hallb/hps/data/physrun2016',
    'RELEASE':'v022317',
    'JAR':'hps-distribution-3.11_v022317.jar',
    'XMLTEMPLATE':'templates/recon.xml',
    'RUNTYPE':'recon',
    'PREFIX':'hps',
    'DEBUG': '0',
...
CANONRUNS=[7636, 7800, 7983, 8028, 8054, 8087, 8100] #,5739,5698]
...
ROOTDIR='/u/group/hps/production/data/PhysRun2016'
    ...
    with open(jsubfilename,'w') as tmp:
        for line in lines:
            for key in PARS.keys():
                ...
                line=line.replace('XXX'+key+'XXX',str(PARS[key]))
            if line.find('CANON')==0:
                if not int(runno) in CANONRUNS and not int(runno) in CALIBRUNS: continue
#                if not int(runno) in CANONRUNS: continue
                line=line.replace('CANON','     ')
            if line.find('PROD')==0:
                if runno in CALIBRUNS: continue
                else: line=line.replace('PROD','    ')
