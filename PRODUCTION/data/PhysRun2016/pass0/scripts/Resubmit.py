...
def getCategorizedFileNumbersOfRun(passn, category, RunNo):
    ...
    if category == 'nt_tri':          stub = '/ntuple/tri'           ; run_type = 'nt_tri'
    if category == 'nt_fee':          stub = '/ntuple/fee'           ; run_type = 'nt_fee'
    if category == 'nt_moller':       stub = '/ntuple/moller'        ; run_type = 'nt_moller'
...
#    s0_miss_filenos = getListAsOneString(s0_missing_number_dict[cur_run])
    ...
    mkjsubs.Remkjsub(cur_run, nt_skim_file_nos, "ntuple")
    ...
    if int(cur_run) in mkjsubs.IGNORERUNS: continue
        ...
        print >>f_miss_stats, cur_run, len(raw_number_dict[cur_run]), len( recon_missing_number_dict[cur_run]), len( dst_missing_number_dict[cur_run]), len( dqm_missing_number_dict[cur_run]), \
            ...
            len( moller_missing_number_dict[cur_run]), len( moller_dst_missing_number_dict[cur_run]), len( v0_missing_number_dict[cur_run]), len( v0_dst_missing_number_dict[cur_run])
