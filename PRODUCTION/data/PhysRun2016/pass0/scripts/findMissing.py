...
def getRunfilesFromXmlFile(fileName):
    ...
    for line in open(fileName,'r').readlines():
        if line.find('<List name="fnum">')>0:
            ...
            if len(filenos)<=0: sys.exit('Kuku1 Error in '+fileName+':  '+line)
        elif line.find('<Variable name="run"')>0:
            mm=re.search('(\d\d\d\d+)',line)
            if mm==None: sys.exit('Kuku2 Error in '+fileName+':  '+line)
...
def getRunfileFromDataFile(fileName):
    ...
    if mm==None: sys.exit('Kuku3 Error in '+fileName)
...
def getNfiles(runfiles):
    nfiles=0
    for runno in runfiles.keys(): nfiles += len(runfiles[runno])
    return nfiles
