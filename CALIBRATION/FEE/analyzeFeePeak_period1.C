...
const double COSMIC_FACTOR = 1.069;
	  ...
	  if (!ishole(jx,jy)){
		...
		if (ITER==1){	  
		  ...
		  case 163: //(2,2)
		    ...
		    break;
		   case 164: //(3,2)
		    MPV[jx][jy]=3.25/EBEAM;
		    break;
		  case 54:   //(-16,4)
		    MPV[jx][jy]=3.064/EBEAM;
		    break;
		  case 130:   //(15,3)
		    MPV[jx][jy]=2.976/EBEAM;
		    break;
		  default:
		    break;
		  }
		...
		if ((calcIY(jy)==-1)||(calcIY(jy)==1))  MPV[jx][jy]=-1;
		if (calcIX(jx)<=-19)  MPV[jx][jy]=-1;
		if (calcIX(jx)>=16)   MPV[jx][jy]=-1;
		if ((calcIY(jy)==-5)||(calcIY(jy)==5))  MPV[jx][jy]=-1;
