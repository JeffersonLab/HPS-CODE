...
#        print "passed basic cuts"
        ...
        for  i in range(0,hps_event.getNumberOfEcalClusters()) :
            ...
            #        continue            
            if not myhist.inSuperFiducialRegion(cl_xi, cl_yi) :
                continue
            if not (cl_ti > clTimeMin and cl_ti < clTimeMax ):
                continue
            if not myhist.momFromPositionEclUpperCut(cl_Ei,myhist.momFromECalPosition(cl_xi,cl_zi,beamAngle,myhist.BEff)) : 
                continue
...
#         //        continue
                if not myhist.inSuperFiducialRegion(cl_xj,cl_yj): 
                    continue
...
#         //   continue
                if not myhist.momFromPositionEclUpperCut(cl_Ej,myhist.momFromECalPosition(cl_xj,cl_zj,beamAngle,myhist.BEff)) : 
                    continue
...
#        if len(pairList) >0 : print "found this many pairs "+str(len(pairList))
        ...
        for pair in pairList : 
            ...
            if Esum>myhist.midESumLow and Esum<myhist.midESumHigh :
                myhist.h_Ecl_midESum_coplanarity.Fill(coplanarity)
...
#            cl_d_bottom= math.sqrt( (botX - phot_nom_x)*(botX - phot_nom_x) + botY*botY )- (60. + 100*(botE - 0.85)*(botE - 0.85) )       
            ...
            if abs(coplanarity-180)<10 : 
                myhist.fillTridentBand(trEle,clEle,trPos,clPos)
