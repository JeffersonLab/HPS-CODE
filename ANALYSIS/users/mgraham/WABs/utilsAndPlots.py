...
class utilsAndPlots:  
    ...
    histDict={}
#    nbins3dEff=25
#    effEcalPositionVsEnergyDenom=np.zeros(nbins3dEff,nbins3dEff,nbins3dEff)
#    effEcalPositionVsEnergyNumer=np.zeros(nbins3dEff,nbins3dEff,nbins3dEff)
...
#            self.midESumLow=self.midESumLow*2.3/1.05
    ...
    def setEnergyScales(self,beamEnergy) : 
        if beamEnergy == 2.3 : 
            self.minClE=0.2
            self.maxClE=1.6
            self.minTrkP=0.2
...
#            self.midESumLow=self.midESumLow*2.3/1.05
    ...
    #  detHalves='TT','TB','BB'
    def fillTwoBody(self,p1,p2,v0,finalState,detHalves,evtWgt) : 
        #calculate stuff
        ...
        if p1.getType()==0: # it's a photon
            mom1=self.makePhotonMomentum(p1Cl)            
        if p2.getType()==0: # it's a photon
            mom2=self.makePhotonMomentum(p2Cl)
        ...
        self.fill1DHistogram(finalState,detHalves,'eSum',self.nbinsSmall,self.minESum,self.maxESum,pV0.E(),evtWgt)
#            
        hname="pairMass"
        val=mass
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minmass,self.maxmass,val,evtWgt)
#
        hname="eDiff"
        val=ediff
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minediff,self.maxediff,val,evtWgt)
#
        hname="clTimeDiff"
        val=p1ClTime-p2ClTime
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mincltimediff,self.maxcltimediff,val,evtWgt)
#
        hname="p1Mom"
        val=p4p1Beam.E()
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minTrkP,self.maxTrkP,val,evtWgt)
#
        hname="p2Mom"
        val=p4p2Beam.E()
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minTrkP,self.maxTrkP,val,evtWgt)
...
#
        ...
        hname="coplanarity"
        val=self.getECalCoplanarity(p1Cl,p2Cl)
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mincopl,self.maxcopl,val,evtWgt)
        ...
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minpt,self.maxpt,val,evtWgt)
#
# this  (above and below) varies from Sho's plot because I'm not using fitted pV0 (just the sum)
        hname="V0PyXSignP1" #this is the same, but now using sign of the positron/gamma ... should not matter!!!
        val=pV0.Py()*np.sign(p4p1Beam.Py())
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minpt,self.maxpt,val,evtWgt)
#
        hname="p1ClusterSlope"
        val=p1Cl.getPosition()[1]/p1Cl.getPosition()[2]
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minslope,self.maxslope,val,evtWgt)
#
        hname="p2ClusterSlope"
        val=p2Cl.getPosition()[1]/p2Cl.getPosition()[2]
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,0,self.maxslope,val,evtWgt)

        ly1=p1Cl.getSeed().getYCrystalIndex()
        lx1=p1Cl.getSeed().getXCrystalIndex()

        ly2=p1Cl.getSeed().getYCrystalIndex()
        lx2=p1Cl.getSeed().getXCrystalIndex()

        hname='p1ClusterEnergyECalY'+str(ly1)
        val=p1Cl.getEnergy()
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minClE,self.maxClE,val,evtWgt)

        hname='p2ClusterEnergyECalY'+str(ly2)
        val=p2Cl.getEnergy()
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minClE,self.maxClE,val,evtWgt)

        
        hname='pTAsymmetry'
        pt1=self.pT(p1.getMomentum())
        pt2=self.pT(p2.getMomentum())
        val=(pt2-pt1)/(pt2+pt1)
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,-0.8,0.8,val,evtWgt)


        if v0 is not None: 
            hname="v0Chi2"
            val=v0.getVertexFitChi2()
            if val>self.maxv0chi2 : val=self.maxv0chi2-0.1
            if val<0 : val=0
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,0,self.maxv0chi2,val,evtWgt)

        if p1.getType()!=0 :#only if p1 is a track
            hname="p1d0"
            val=p1Trk.getD0()
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mind0,self.maxd0,val,evtWgt)
#
            hname="p1z0"
            val=p1Trk.getZ0()
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minz0,self.maxz0,val,evtWgt)
#
            hname="p1phi0"
            val=p1Trk.getPhi0()
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minphi0,self.maxphi0,val,evtWgt)
#
            hname="p1slope"
...
#
            ...
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mintrktime,self.maxtrktime,val,evtWgt) 
#
            hname="p1TrkChi2overNHits"
            val=p1Trk.getChi2()/len(p1Trk.getSvtHits())
            if p1Trk.getChi2() <0 or p1Trk.getChi2()>50: 
                print 'p1Trk.getChi2() = '+str(p1Trk.getChi2())
            if val>self.maxtrkchi2 : val=self.maxtrkchi2-0.1
            if val<self.mintrkchi2 : val=self.mintrkchi2

            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mintrkchi2,self.maxtrkchi2,val,evtWgt) 
#
...
#
            ...
            self.fill2DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minTrkP,self.maxTrkP,
                                self.nbinsSmall,self.minClE,self.maxClE,valx,valy)
#
            hname="p1MissingLayer"
            val=self.getMissingLayer(p1Trk)
            self.fill1DHistogram(finalState,detHalves,hname,7,0,7,val,evtWgt) 
...
#
            ...
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mintrktime,self.maxtrktime,val,evtWgt)
#
            hname="p2TrkChi2overNHits"
            val=p2Trk.getChi2()/len(p2Trk.getSvtHits())
            if val>self.maxtrkchi2 : val=self.maxtrkchi2-0.1
            if val<self.mintrkchi2 : val=self.mintrkchi2
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mintrkchi2,self.maxtrkchi2,val,evtWgt) 
#
...
#
            ...
            self.fill2DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minTrkP,self.maxTrkP,
                                self.nbinsSmall,self.minClE,self.maxClE,valx,valy)
#
            hname="p2MissingLayer"
            val=self.getMissingLayer(p2Trk)
            self.fill1DHistogram(finalState,detHalves,hname,7,0,7,val,evtWgt) 
#
...
#
            ...
            self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.mintrktimediff,self.maxtrktimediff,val,evtWgt)
    ...
    def fillThreeBody(self,p1,p2,p3,finalState,detHalves) : 
        ...
        if p3.getType()==0: # it's a photon
            mom3=self.makePhotonMomentum(p3Cl)
...
#
        hname="threeBodyeSum"
        val=pV0.E()
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minESum,self.maxESum,val,evtWgt)
# 
        hname="threeBodypT"
        val=pV0.Pt()
        self.fill1DHistogram(finalState,detHalves,hname,self.nbinsSmall,self.minpt,self.maxpt,val,evtWgt)
#
        ...
        self.fillTwoBody(p1,p2,None,finalState+"-p1p2",detHalves)
        self.fillTwoBody(p1,p3,None,finalState+"-p1p3",detHalves)
        self.fillTwoBody(p2,p3,None,finalState+"-p2p3",detHalves)
    ...
    def book1DHistogram(self,fs,dh,hname,nbins,minX,maxX) : 
        ...
        self.histogramList[-1].Sumw2()         
        self.histDict[fs][dh][hname]=self.histogramList[-1]
                    
    def fill1DHistogram(self,fs,dh,hname,nbins,minX,maxX,val,evtWgt) : 
        try :
            hist=self.histDict[fs][dh][hname]; 
        except KeyError, e:
            self.book1DHistogram(fs,dh,hname,nbins,minX,maxX)            
        self.histDict[fs][dh][hname].Fill(val,evtWgt)
...
#########
    def simpleClusterCoincidence(self, p1,p2, cutVal) : 
        if p1.getClusters().GetEntries == 0 : 
            return False
        if p2.getClusters().GetEntries == 0 : 
            return False
        ...
        p1ClTime=p1Cl.getClusterTime()
        p2ClTime=p2Cl.getClusterTime()
        if abs(p1ClTime-p2ClTime)>cutVal : 
            return False
        return True
...
#########   remove a hit in layer X
#########   mg 11/18/2017...this del doesn't seem to work...the length after del is the same as before
    @staticmethod
    def removeLXHit(trk,layer): 
        if trk == None : 
            return False
        hitList=trk.getSvtHits()
        print 'removing hit in layer = '+str(layer)        
        for i in range(0,len(hitList)) : 
            if hitList[i].getLayer() == layer : 
#                print 'removing hit!'
                print len(hitList)
                print 'layer = '+str(hitList[i].getLayer())
                del hitList[i]
                print hitList
#                print len(hitList)
#                print 'layer = '+str(hitList[i].getLayer())
...
####  return layer that hit is missing...return 0 if all 6 hits 
    def getMissingLayer(self,trk) :
        if trk==None: 
            return 0
        for l in range(1,7) : 
            if not self.hasLXHit(trk,l): 
                return l
        return 0
            
######    cluster-track timing coincidence 
    def clusterTrackCoincidence(self,p1,cut,offset) : 
        if p1.getClusters().GetEntries == 0 : 
            return False
        if p1.getTracks().GetEntries == 0 : 
            return False
        p1ClTime=p1.getClusters().First().getClusterTime()
        p1TrackTime=p1.getTracks().First().getTrackTime()
         
        if abs(p1ClTime-p1TrackTime-offset)>cut : 
            return False
        
        return True
        
######  ECal coplanarity cut
    def cutCoplanarity(self,p1,p2,cut,offset) : 
        if p1.getClusters().GetEntries == 0 : 
            return False
        if p2.getClusters().GetEntries == 0 : 
            return False

        if abs(self.getECalCoplanarity(p1.getClusters().First(),p2.getClusters().First())-offset)>cut : 
            return False
