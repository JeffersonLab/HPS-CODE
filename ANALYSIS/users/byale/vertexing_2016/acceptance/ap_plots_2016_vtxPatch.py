...
exppol4=TF1("exppol4","min(exp(pol3(0)),2)",0,70)
#exppol4=TF1("exppol4","min(exp(pol4(0)),2)",0,70)

c.Clear()
c.Divide(1,2)
c.cd(1)
gPad.SetLogy(1)
events2.Draw("triEndZ>>prodZ(50,0.5,100)","triP>0.8*2.3","colz,goff")
...
c.cd(1)
#gPad.SetLogy(1)
events.Draw("triEndZ>>hnew(50,0.5,100)","triP>0.8*2.3&&uncP>0.8*2.3","colz,goff")
hnew = gDirectory.Get("hnew")
...
c.cd(1)
gPad.SetLogz(1)
events.Draw("uncVZ-triEndZ:triEndZ>>hnew(50,0.5,100,50,-20,20)","triP>0.8*2.3&&uncP>0.8*2.3","colz")
hnew = gDirectory.Get("hnew")
hnew.GetXaxis().SetTitle("vertex Z [mm]");
...
c.cd(1)
gPad.SetLogz(1)
events.Draw("mRefitUnc-triM:triEndZ>>hnew(50,0,100,50,-0.02,0.02)","triP>0.8*2.3&&uncP>0.8*2.3","colz")
hnew = gDirectory.Get("hnew")
...
if isFullTuple:
    ...
    c.Divide(1,2)
    c.cd(1)
    events.Draw("mRefitUnc-triM>>hnew_1(50,-0.02,0.02)","triP>0.8*2.3&&uncP>0.8*2.3&&abs(triEndZ-30)<5","colz")
    hnew_1 = gDirectory.Get("hnew_1")
    hnew_1.GetXaxis().SetTitle("mass residual [GeV]");
    #hnew_1.GetYaxis().SetTitle("mass resolution [GeV]");
    hnew_1.Draw()
    c.cd(2)
    events.Draw("mRefitUnc-triM:triEndZ>>hnew(50,-0.1,0.1,50,-0.02,0.02)","triP>0.8*2.3&&uncP>0.8*2.3&&abs(triEndZ-30)<5","colz")
    hnew = gDirectory.Get("hnew")
    ...
    c.cd(1)
    gPad.SetLogz(1)
    #events.Draw("mRefitUnc-0.15e-3*(elePX/eleP-posPX/posP)*triEndZ/triM-triM:triEndZ>>hnew(50,0,100,50,-0.02,0.02)","triP>0.8*2.3&&uncP>0.8*2.3","colz")
    events.Draw("mRefitUnc-triM:triEndZ>>hnew(50,0,100,50,-0.02,0.02)","triP>0.8*2.3&&uncP>0.8*2.3","colz")
    hnew = gDirectory.Get("hnew")
    hnew.GetXaxis().SetTitle("vertex Z [mm]");
    hnew.GetYaxis().SetTitle("mass residual [GeV]");
