...
#events.Print()
#events.Draw("uncVZ:{0}>>hnew(100,0,0.17,100,-50,50)".format(massVar),"","colz")
events.Draw("{1}:{0}>>hnew(100,0,0.17,100,-5,20)".format(massVar,vtxVar),"","colz")
...
w.factory("EXPR::gaussExp('exp( ((@0-@1)<@3)*(-0.5*(@0-@1)^2/@2^2) + ((@0-@1)>=@3)*(-0.5*@3^2/@2^2-(@0-@1-@3)/@4))',{0},gauss_mean[0,-20,20],gauss_sigma[5,1,50],exp_breakpoint[10,0,50],exp_length[3,0.5,20])".format(vtxVar))
...
for i in range(0,n_massbins):
    ...
    dataInRange = dataset.reduce(obs,"abs({0}-{1})<{2}/2*({3}+{4}*{5})".format(massVar,mass,masscut_nsigma,mres_p0,mres_p1,vtxVar))
...
#
    ...
    print "vertex sigma "+str(sigma)
    dataPastCut2 = dataInRange.reduce(w.set("obs_1d"),"{1}>{0}".format(tailcut,vtxVar)).binnedClone()
...
#    bkgHist.Draw()
    ...
    for j in range(0,n_epsbins):
        ...
        sig_integral = exppol4.Integral(targetz,maxz)
        ...
        cdfAtZcut = exppol4.Integral(zcut,maxz)
        if (no_candidates):
...
c.SetLogx(0)
events.Draw("{1}:{0}>>hnew({2},0,0.17,180,-5,20)".format(massVar,vtxVar,n_massbins),"highzcut","colz")
