...
fitBackground = True
nEvents = 15
pdf = TF1("eventpdf","max(0,pol2)",0.040,0.130)
...
mres = TF1("mres","pol1",0.040,0.130)
...
pdfIntegral = pdf.IntegralOneDim(0.0,0.17,1e-12,1e-12,ROOT.Double(blahh))

if (nEvents==0):
    nEvents = pdfIntegral*0.5/2.8
print nEvents

fullRange = pdf.IntegralOneDim(0.040 - 1.4*mres.Eval(0.02),0.130+1.4*mres.Eval(0.06),1e-12,1e-12,ROOT.Double(blahh))/2.8
