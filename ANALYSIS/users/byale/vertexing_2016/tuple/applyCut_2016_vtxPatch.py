...
if tweakVertex:
    #corrM = events["mRefitUnc"]-0.15e-3*(events["elePX"]/events["eleP"]-events["posPX"]/events["posP"])*events["uncVZ"]/events["mRefitUnc"]
    corrM = events["mRefitUnc"]
    stuff.append([corrM,("corrM",events.dtype["mRefitUnc"])])
...
for i in xrange(0,n):
    if events[i]["event"]!=currentevent:
        if cutType=="bumphunt":
            ...
            candidates.sort(key=lambda x:events[x]["chisqFitBSC"],reverse=False)
        elif cutType=="none":
            candidates.sort(key=lambda x:events[x]["tarChisq"],reverse=False)
        else:
            raise Exception("invalid cut type")
