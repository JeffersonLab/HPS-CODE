...
echo "Installing to $prefix/${INSTALL_ARCH}/lib and bin"
mkdir -p  $prefix/${INSTALL_ARCH}/lib
mkdir -p  $prefix/${INSTALL_ARCH}/bin
cp lib/libevio*.so $prefix/${INSTALL_ARCH}/lib
cp lib/libevio*.a $prefix/${INSTALL_ARCH}/lib
cp bin/evio2xml   $prefix/${INSTALL_ARCH}/bin

if [ ${ARCH} = "Darwin" ]; then
install_name_tool -id  $prefix/${INSTALL_ARCH}/lib/libevio.so $prefix/${INSTALL_ARCH}/lib/libevio.so
install_name_tool -id  $prefix/${INSTALL_ARCH}/lib/libevioxx.so $prefix/${INSTALL_ARCH}/lib/libevioxx.so
install_name_tool -change lib/libevio.so $prefix/${INSTALL_ARCH}/lib/libevio.so $prefix/${INSTALL_ARCH}/lib/libevioxx.so
install_name_tool -change lib/libevio.so $prefix/${INSTALL_ARCH}/lib/libevio.so $prefix/${INSTALL_ARCH}/bin/evio2xml
install_name_tool -change lib/libevioxx.so $prefix/${INSTALL_ARCH}/lib/libevioxx.so $prefix/${INSTALL_ARCH}/bin/evio2xml
fi
