...
void EvioEventInit(EVIO_Event_t *evt){
  ...
  evt->FADC_21.clear();
  evt->FADC_21.reserve(MAX_NUM_FADC);
  
  evt->SSP_data.clusters.clear();
  evt->SSP_data.clusters.reserve(MAX_NUM_CLUSTERS);
  evt->SSP_data.singles.clear();
  evt->SSP_data.singles.reserve(MAX_NUM_CLUSTERS);
  evt->SSP_data.pairs.clear();
  evt->SSP_data.pairs.reserve(MAX_NUM_CLUSTERS*10);
