...
#events.Print()
events.Draw("uncVZ:{0}>>hnew(100,0,0.1,100,-60,60)".format(massVar),"","colz")
...
fz1 = TF1("fz1","(x>0.025)*(-34.7156+8918.04*x-385353*pow(x,2.0)+7.48106E6*pow(x,3.0)-6.99006E7*pow(x,4.0)+2.55321E8*pow(x,5.0))+(x<=0.025)*(95.47-7068.91*x+195194.0*pow(x,2.0))",0.018,0.07)

fp0 = TF1("fp0","-215.132+9762.0*x-152023.0*pow(x,2.0)+792141*pow(x,3.0)",0.018,0.07)
fp1 = TF1("fp1","29.1635+1321.43*x-81834.1*pow(x,2.0)+1.37187E6*pow(x,3.0)-7.46855E6*pow(x,4.0)",0.018,0.07)
fp2 = TF1("fp2","16.4247-43.7106*x-9425.11*pow(x,2.0)+196389.0*pow(x,3.0)-1.14209E6*pow(x,4.0)",0.018,0.07)
...
#    bkgHist.Draw()
    ...
    c.Clear()
    c.SetLogy()
    uncVZ.setRange("fitRange",tailcut,60)
    frame=uncVZ.frame()
    frame.SetAxisRange(tailcut,60)
    frame.SetTitle(name)
    frame.GetXaxis().SetTitle("Vertex Z [mm]")
    ...
    #bkgNLL = fitresult.minNll() #temporarily get rid of this
    ...
    for j in range(0,n_epsbins):
        ...
        for binnum in xrange(1,nbins_histpdf+1):
            ...
            if sigHist.GetXaxis().GetBinCenter(binnum)>=targetz:
                ...
                sigHist.SetBinContent(binnum,effHisto.GetBinContent(effHisto.GetXaxis().FindBin(sigHist.GetXaxis().GetBinCenter(binnum))))
        #sigHist.Draw()
        ...
        #fitresult = thispdf.fitTo(dataPastCut2,RooFit.Range("fitRange"),RooFit.PrintLevel(-1),RooFit.Save()) #temporarily get rid of this
        #sigNLL = fitresult.minNll() #temporarily get rid of this
        #thispdf.plotOn(frame,RooFit.Range("fitRange"),RooFit.NormRange("fitRange"))
        #thispdf.paramOn(frame)
        #frame.SetMinimum(0.1)
        #frame.Draw()
        #c.Print(remainder[0]+".pdf","Title:test2")
        #print "bkg-only NLL {0}, sig+bkg NLL {1}".format(bkgNLL,sigNLL) #temporarily get rid of this
        #logplrHist.Fill(mass,10**eps,bkgNLL-sigNLL) #temporarily get rid of this
...
c.SetLogx(0)
events.Draw("uncVZ:{0}>>hnew(100,0,0.1,100,-60,60)".format(massVar),"highzcut","colz")
