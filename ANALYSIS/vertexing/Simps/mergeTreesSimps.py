...
simpCut = "isPair1&&eleHasL1&&posHasL1&&max(eleMatchChisq,posMatchChisq)<10&&max(abs(eleClT-eleTrkT-{1}),abs(posClT-posTrkT-{1}))<4&&abs(eleClT-posClT)<2&&eleClY*posClY<0&&bscChisq<10&&bscChisq-uncChisq<5&&max(eleTrkChisq,posTrkChisq)<30&&min(eleMinPositiveIso+0.5*(eleTrkZ0+{2}*elePY/eleP)*sign(elePY),posMinPositiveIso+0.5*(posTrkZ0+{2}*posPY/posP)*sign(posPY))>0&&eleP<{0}*0.4&&posP<{0}*0.4&&uncP<{0}*0.65&&uncP>{0}*0.4"
simpCutAp = "tarP>1.51&&posTrkD0+{2}*posPX/posP<1.01&&posHasL1&&max(eleTrkChisq,posTrkChisq)<70&&abs(eleClT-posClT)<2&&max(abs(eleClT-eleTrkT-{1}),abs(posClT-posTrkT-{1}))<4.4&&tarP<2.9&&eleP<1.76&&eleClY*posClY<0&&max(eleMatchChisq,posMatchChisq)<10"
cut=""

options, remainder = getopt.gnu_getopt(sys.argv[1:], 'sbtpolVvamwe:c:z:h')
