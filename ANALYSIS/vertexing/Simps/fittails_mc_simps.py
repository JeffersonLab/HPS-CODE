...
for i in range(0,n_massbins):
    ...
    zeroArr.append(0)
    mres_p0 = acceptanceFile.Get("mres").GetFunction("pol1").Eval(mass)
    mres_p1 = 0
    c.Clear()
    c.Divide(1,2)
    c.cd(1)
    events.Draw("uncVZ:{0}>>hnew2d(100,0,0.2,100,-50,50)".format(massVar),"abs({0}-{1})<{2}/2*({3}+{4}*uncVZ)".format(massVar,mass,masscut_nsigma,mres_p0,mres_p1),"colz")
    c.cd(2)
    gPad.SetLogy(1)
...
graph.Draw("A*")
graph.SetTitle("Vertex Z Resolution")
graph.GetXaxis().SetTitle("mass [GeV]")
graph.GetYaxis().SetTitle("vertex resolution [mm]")
graph.Write("sigmaz")
