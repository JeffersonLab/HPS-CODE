...
CRATES=[37,39]
SLOTS=[3,4,5,6,7,8,9,14,15,16,17,18,19,20]
CHANS=range(16)
XVALS,YVALS=[],[]
PVS={}
for crate in CRATES:
    for slot in SLOTS:
        for chan in CHANS:
            xy=ECAL.fadc2xy(crate,slot,chan)
            xx=xy[0]
            if xx<0: xx+=1
            XVALS.append(xx)
            YVALS.append(xy[1])
for crate in CRATES:
    cc={}
    if   crate==37: icrate=1
    elif crate==39: icrate=2
    for slot in SLOTS:
        ss={}
        for chan in CHANS:
            ss[chan]=pv.PV('B_SCALERSHPS%d_Sl%d_Ch%d.VAL'%(icrate,slot,chan))
        cc[slot]=ss
    PVS[crate]=cc
...
def setupHists(hhh):
    for hh in hhh:
        ...
        hh.GetXaxis().SetNdivisions(46,0)
        hh.GetYaxis().CenterLabels()
        hh.GetYaxis().SetNdivisions(11,0)
